<div>
    <div class="flex items-center justify-between mb-6">
        ...
        <a href="{{ route('admin.projects.create') }}" class="px-4 py-2 bg-brand-600 hover:bg-brand-700 rounded-lg text-sm font-medium transition">Nuevo proyecto</a>
    </div>

    <div class="flex gap-4 mb-6">
        <x-text-input wire:model.live="search" placeholder="Buscar proyectos..." class="w-64" />
        ...
        <table class="w-full text-sm">
            <thead class="bg-surface-card border-b border-surface-border">
                <tr>
                    ...
                    <th class="text-left py-3 px-4 text-gray-400 font-medium">Año</th>
                    <th class="text-left py-3 px-4 text-gray-400 font-medium">Destacado</th>
            ...
            <tbody class="divide-y divide-surface-border">
                @forelse ($projects as $project)
                    <tr class="hover:bg-surface-hover transition">
                        ...
                        <td class="py-3 px-4">
                            <span class="px-2 py-0.5 text-xs rounded-full {{ $project->status === 'published' ? 'bg-green-500/10 text-green-400' : 'bg-yellow-500/10 text-yellow-400' }}">
                                {{ $project->status }}
                            </span>
                        </td>
                        ...
                        <td class="py-3 px-4 text-right">
                            <a href="{{ route('admin.projects.edit', $project) }}" class="text-brand-400 hover:text-brand-300 transition text-sm mr-3">Editar</a>
